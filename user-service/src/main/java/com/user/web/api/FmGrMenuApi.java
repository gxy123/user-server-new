 ...
 */
@Api(description = "菜单表共三级-相关接口")
@Slf4j
@RestController
@RequestMapping("/api/fmgrmenu")
public class FmGrMenuApi extends BaseControllerImpl<FmGrMenuDO, FmGrMenuQueryDO> {

    @Autowired
    private FmGrMenuService baseService;

    @Override
    public BaseServiceAO<FmGrMenuDO, FmGrMenuQueryDO> getService() {
        return baseService;
    }
    /**
     * 通用查询逻辑
    ...
    public CommonResult<List<FmGrMenuDO>> select(@ModelAttribute("pojo") FmGrMenuQueryDO q) {
        CommonResult<List<FmGrMenuDO>> query = getService().query(q);
    ...
    @RequestMapping("get")
    public CommonResult<FmGrMenuDO> detail(@RequestParam("id") Long id) {
    ...
    public CommonResult<Long> update(@ModelAttribute("pojo") FmGrMenuDO t) {
    ...
    @RequestMapping("add")
    public CommonResult<FmGrMenuDO> insert(@RequestBody FmGrMenuDO t) {
