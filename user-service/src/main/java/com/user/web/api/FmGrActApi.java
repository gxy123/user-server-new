 ...
 */
@Api(description = "接口-相关接口")
@Slf4j
@RestController
@RequestMapping("/api/fmgract")
public class FmGrActApi extends BaseControllerImpl<FmGrActDO, FmGrActQueryDO> {

    @Autowired
    private FmGrActService baseService;

    @Override
    public BaseServiceAO<FmGrActDO, FmGrActQueryDO> getService() {
        return baseService;
    }
    /**
     * 通用查询逻辑
    ...
    public CommonResult<List<FmGrActDO>> select(@ModelAttribute("pojo") FmGrActQueryDO q) {
        CommonResult<List<FmGrActDO>> query = getService().query(q);
    ...
    @RequestMapping("get")
    public CommonResult<FmGrActDO> detail(@RequestParam("id") Long id) {
    ...
    public CommonResult<Long> update(@ModelAttribute("pojo") FmGrActDO t) {
    ...
    @RequestMapping("add")
    public CommonResult<FmGrActDO> insert(@RequestBody FmGrActDO t) {
