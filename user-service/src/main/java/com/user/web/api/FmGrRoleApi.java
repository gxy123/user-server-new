 ...
 */
@Api(description = "角色表-相关接口")
@Slf4j
@RestController
@RequestMapping("/api/fmgrrole")
public class FmGrRoleApi extends BaseControllerImpl<FmGrRoleDO, FmGrRoleQueryDO> {

    @Autowired
    private FmGrRoleService baseService;

    @Override
    public BaseServiceAO<FmGrRoleDO, FmGrRoleQueryDO> getService() {
        return baseService;
    }
    /**
     * 通用查询逻辑
    ...
    public CommonResult<List<FmGrRoleDO>> select(@ModelAttribute("pojo") FmGrRoleQueryDO q) {
        CommonResult<List<FmGrRoleDO>> query = getService().query(q);
    ...
    @RequestMapping("get")
    public CommonResult<FmGrRoleDO> detail(@RequestParam("id") Long id) {
    ...
    public CommonResult<Long> update(@ModelAttribute("pojo") FmGrRoleDO t) {
    ...
    @RequestMapping("add")
    public CommonResult<FmGrRoleDO> insert(@RequestBody FmGrRoleDO t) {
